...
ua.base 전체(100%)로 학습 후 각 Stage 모델 저장
"""
import os
import time
import numpy as np
...
# ============================================
print("\n─" * 65)
print("[Stage 1] Basic MF — r̂ = μ + P[u]·Q[i], 정규화 없음")
start = time.time()
...
np.savez(f'{MODEL_DIR}/mf_stage1_k{K}.npz', P=P1, Q=Q1, mu=np.array([mu]))
...
# ============================================
print("\n─" * 65)
print(f"[Stage 2] + L2 정규화 (λ={REG}) — 과적합 방지")
start = time.time()
...
for epoch in range(EPOCHS):
    ...
    for j in order:
        u, i, r = users[j], items[j], ratings[j]
        err = r - (mu + P2[u] @ Q2[i])
        P2[u] += LR * (err * Q2[i] - REG * P2[u])    # +정규화
        Q2[i] += LR * (err * P2[u] - REG * Q2[i])    # +정규화
...
np.savez(f'{MODEL_DIR}/mf_stage2_k{K}.npz', P=P2, Q=Q2, mu=np.array([mu]))


# ============================================
# Stage 3: + Bias (μ + bu + bi)
# ============================================
print("\n─" * 65)
print("[Stage 3] + Bias terms (μ, bu, bi) — 사용자/아이템 편향 모델링")
start = time.time()
...
for epoch in range(EPOCHS):
    ...
    for j in order:
        u, i, r = users[j], items[j], ratings[j]
        err = r - (mu + bu3[u] + bi3[i] + P3[u] @ Q3[i])    # +bias
        bu3[u] += LR * (err - REG * bu3[u])                   # bias 업데이트
...
np.savez(f'{MODEL_DIR}/mf_stage3_k{K}.npz', P=P3, Q=Q3, bu=bu3, bi=bi3, mu=np.array([mu]))
...
Y4  = np.random.normal(0, 0.1, (N_ITEMS, K))     # implicit vector
...
for epoch in range(EPOCHS):
    ...
    for j in order:
        ...
        p_aug = P4[u] + norm * Y4[I_u].sum(axis=0)   # P_augmented
        ...
        Y4[I_u] += LR * (err * norm * Q4[i] - REG * Y4[I_u])  # implicit 업데이트
...
np.savez(f'{MODEL_DIR}/mf_stage4_k{K}.npz', P=P4, Q=Q4, Y=Y4, bu=bu4, bi=bi4, mu=np.array([mu]))
...
print(f"  {'Stage 3':>10} {'+ Bias (μ,bu,bi)':>30} {s3_rmse:>8.4f} {s2_rmse - s3_rmse:>+10.4f}")
...
print(f"    mf_stage1_k{K}.npz ~ mf_stage4_k{K}.npz")
print(f"\n{'=' * 65}")
print("학습 완료! 다음 단계: python mf_test.py")
print(f"{'=' * 65}")
