...
저장된 4단계 모델을 불러와서 ua.test로 최종 RMSE/MAE 측정
...
# ============================================
# 1. ua.base 로드 (user_rated_items 재구성용, 학습 아님!)
# ============================================
...
print(f"  학습 데이터 (ua.base): {len(train_df)}개 평점 — P_aug 계산용으로만 사용")

# ============================================
# 2. ua.test 로드 (⚠️ 이 스크립트에서만 사용!)
# ============================================
...
print(f"  테스트 데이터 (ua.test): {len(test_df)}개 평점")
...
# --- Stage 1: Basic MF ---
path1 = f'{MODEL_DIR}/mf_stage1_k{K}.npz'
if os.path.exists(path1):
    ...
    print(f"  ✅ Stage 1 모델 로드: {path1}")
else:
    print(f"  ❌ Stage 1 모델 없음: {path1}")

# --- Stage 2: + L2 ---
path2 = f'{MODEL_DIR}/mf_stage2_k{K}.npz'
if os.path.exists(path2):
    ...
    print(f"  ✅ Stage 2 모델 로드: {path2}")
else:
    print(f"  ❌ Stage 2 모델 없음: {path2}")

# --- Stage 3: + Bias ---
path3 = f'{MODEL_DIR}/mf_stage3_k{K}.npz'
if os.path.exists(path3):
    ...
    results.append(('Stage 3', '+ Bias (μ,bu,bi)', rmse3, mae3, pred3))
    print(f"  ✅ Stage 3 모델 로드: {path3}")
else:
    print(f"  ❌ Stage 3 모델 없음: {path3}")

# --- Stage 4: + Implicit (SVD++) ---
path4 = f'{MODEL_DIR}/mf_stage4_k{K}.npz'
if os.path.exists(path4):
    ...
    print(f"  ✅ Stage 4 모델 로드: {path4}")
else:
    print(f"  ❌ Stage 4 모델 없음: {path4}")
...
if len(results) >= 5:
    ...
    print(f"\n  총 개선: Baseline({results[0][2]:.4f}) → Stage 4({results[-1][2]:.4f}) = {total_improve:.4f} ({total_improve/results[0][2]*100:.1f}%)")

# ============================================
# 5. Stage 4 Grouping 분석
# ============================================
if len(results) >= 5:
    final_pred = results[-1][4]  # Stage 4 예측값
    ...
    for grp in ['Cold (<30)', 'Medium (30-80)', 'Active (80+)']:
        ...
        if len(g) == 0:
            continue
        g_rmse = np.sqrt(g['sq_err'].mean())
        g_mae = g['abs_err'].mean()
        n_users = g['user_id'].nunique()
        print(f"  {grp:>18} {n_users:>8} {g_rmse:>8.4f} {g_mae:>8.4f}")
    ...
    for stage, desc, rmse, mae, _ in results:
        summary_rows.append({
            ...
            'data': 'ua.test', 'RMSE': round(rmse, 4), 'MAE': round(mae, 4)
        })
    summary_df = pd.DataFrame(summary_rows)
    summary_path = f'{OUTPUT_DIR}/test_mf_stages_summary.csv'
    summary_df.to_csv(summary_path, index=False)

    print(f"\n  예측 결과 저장: {result_path}")
    print(f"  단계별 요약 저장: {summary_path}")
