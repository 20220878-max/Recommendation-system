...
def get_lost_items(db: Session, skip: int = 0, limit: int = 20, category=None):
    q = db.query(LostItem)
    if category:
        q = q.filter(LostItem.category == category.value)
...
def get_found_items(db: Session, skip: int = 0, limit: int = 20, category=None):
    q = db.query(FoundItem)
    if category:
        q = q.filter(FoundItem.category == category.value)
    return q.offset(skip).limit(limit).all()
